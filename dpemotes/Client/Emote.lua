...
AddEventHandler("onKeyDown", function(key)
    if key == "f3" then
        if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end
        OpenEmoteMenu()
    elseif key == "x" then
        EmoteCancel()
    elseif key == "f" and GetSelectedPedWeapon(PlayerPedId()) == `weapon_unarmed` then
        deleteNearObject()
    end
end)
...
RegisterCommand('e', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end EmoteCommandStart(source, args, raw) end)
RegisterCommand('emote', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end EmoteCommandStart(source, args, raw) end)
if Config.SqlKeybinding then
  RegisterCommand('emotebind', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end EmoteBindStart(source, args, raw) end)
  RegisterCommand('emotebinds', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end moteBindsStart(source, args, raw) end)
end
RegisterCommand('emotemenu', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end OpenEmoteMenu() end)
RegisterCommand('emotes', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end EmotesOnCommand() end)
RegisterCommand('walk', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end WalkCommandStart(source, args, raw) end)
RegisterCommand('walks', function(source, args, raw) if Ban then return ESX.Alert('Error','Shoma mahrum az kar ba emote shodid',5000,'error') end WalksOnCommand() end)
...
function EmoteCancel(force)
  if not disableCancelEmote or force then
...
function deleteNearObject2()
  local objects,distance = ESX.Game.GetClosestObject()
  -- ESX.SetPlayerState('deleteLocalObjects', true)
  -- SetTimeout(200, function()
  --   ESX.SetPlayerState('deleteLocalObjects', false)
  -- end)
  if distance < 1 then
      if IsEntityAttachedToAnyPed(objects) and IsEntityAttachedToEntity(objects,PlayerPedId()) and Entity(objects).state.emote then
          ESX.Game.DeleteObject(objects)
      end
  end
end

AddStateBagChangeHandler('deleteLocalObjects', nil, function(bagName, key, value)
  local ped = GetPlayerPed(GetPlayerFromStateBagName(bagName))
  if ped == 0 or ped == PlayerPedId() then return end
  if value then
    local coords = GetEntityCoords(ped)
    for k, v in pairs(ESX.Game.GetObjects()) do
      if #(coords - GetEntityCoords(v)) < 1 and not Entity(v).state.antiDelete and not NetworkGetEntityIsNetworked(v) and IsEntityStatic(v) then
...
function EmoteMenuStart(args, hard, job)
    local name = args
    local etype = hard
    ...
    elseif etype == "props" then
        if job then
            if DP.jobEmotes[ESX.GetPlayerData().job.name][name] ~= nil then
                if OnEmotePlay(DP.jobEmotes[ESX.GetPlayerData().job.name][name]) then end
            end
        else
...
local ScSpam = false
local tid = 0
function OnEmotePlay(EmoteName,force,xop, disablex)
  if not disablex then
    exports['essentialmode']:disablecontrol('x',true)
  end
  local item = EmoteName.AnimationOptions and EmoteName.AnimationOptions.item or EmoteName[4]
  if item then
    if not ESX.DoesHaveItem2(item, 1) then return ESX.ShowNotification(('Shoma baraye ejraye in emote niaz be %s darid'):format(ESX.getItemLabel(item))) else
      TriggerServerEvent('items:remove', item, 1)
    end
  end
  ...
  if (not Config.AllowedInCars and InVehicle == 1) or IsPedRagdoll(PlayerPedId()) then
    return
  end
  ...
  if ChosenDict == "MaleScenario" or "Scenario" then 
    ...
    if ChosenDict == "MaleScenario" then 
      ...
      if ScSpam then return ESX.ShowNotification('Spam nakonid!') end
      ScSpam = true
      Citizen.SetTimeout(5000,function()
        ScSpam = false
      end)
...
function PlayEmote(emote,force,xop, disablex)
  if force == nil then
    force = false
  end
  local emoteJob = nil
  for k, v in pairs(DP.jobEmotes) do
    if v[emote] then
      emoteJob = v[emote]
      break
    end
  end
  OnEmotePlay(DP.Emotes[emote] or DP.PropEmotes[emote] or emoteJob,force,xop, disablex)
end

...
function AddEmoteMenu(menu)
    ...
    table.insert(EmoteTable, Config.Languages[lang]['danceemotes'])
    if DP.jobEmotes[ESX.GetPlayerData().job.name] then
      jobMenu = _menuPool:AddSubMenu(submenu, '💼Job Emotes', "", "", Menuthing, Menuthing)
      table.insert(EmoteTable, '💼Job Emotes')
      table.insert(EmoteTable, '💼Job Emotes')
    end
    ...
    if jobMenu then
      for a,b in pairsByKeys(DP.jobEmotes[ESX.GetPlayerData().job.name]) do
        x,y,z = table.unpack(b)
        propitem = NativeUI.CreateItem(z, "/e ("..a..")")
        jobMenu:AddItem(propitem)
        table.insert(jobETable, a)
        if not Config.SqlKeybinding then
          propfavitem = NativeUI.CreateItem(z, Config.Languages[lang]['set']..z..Config.Languages[lang]['setboundemote'])
          favmenu:AddItem(propfavitem)
          table.insert(FavEmoteTable, a)
    ...
    if jobMenu then
      jobMenu.OnItemSelect = function(sender, item, index)
        EmoteMenuStart(jobETable[index], "props", true)
      end
    end
...
Citizen.CreateThread(function()
  while not ESX or not ESX.GetPlayerData().job do Wait(1000) end

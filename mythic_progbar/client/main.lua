...
function Progress(action, finish)
    ...
    if mythic_action.duration and mythic_action.duration > 500 then
        SetTimeout(mythic_action.duration - 300, function()
            mythic_action.canEnd = true
        end)
    end
    ...
    Citizen.CreateThread(function()
        while isDoingAction do
            ...
            if not isProp and mythic_action.prop ~= nil and mythic_action.prop.model ~= nil then
                ...
                DecorSetBool(modelSpawn,'prog',true)
            ...
            DisableActions(PlayerPedId(), config)
        end
    end)
end
...
function DisableActions(ped, config)
    if config.controlDisables.disableMouse then
        DisableControlAction(0, 1, true) -- LookLeftRight
        DisableControlAction(0, 2, true) -- LookUpDown
        DisableControlAction(0, 106, true) -- VehicleMouseControlOverride
    end

    if config.controlDisables.disableMovement then
    ...
    if config.controlDisables.disableCarMovement then
    ...
    if config.controlDisables.disableCombat then
...
RegisterNUICallback('actionFinish', function(data, cb)
    -- Do something here
    if mythic_action.canEnd then
        isDoingAction = false
        TriggerEvent("mythic_progbar:client:actionCleanup")
        cb('ok')
    else
        -- TriggerServerEvent('sc:adminalarm', 'Try iframe')
    end
end)

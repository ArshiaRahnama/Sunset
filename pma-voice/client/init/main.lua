...
if gameVersion == 'fivem' then
	...
	-- local callEffectId = CreateAudioSubmix('Phone')
	-- SetAudioSubmixOutputVolumes(
	-- 	callEffectId,
	-- 	1,
	...
	-- submixIndicies['radio'] = CreateAudioSubmix('Radio')
	-- SetAudioSubmixEffectRadioFx(submixIndicies['radio'], 0)
	-- SetAudioSubmixEffectParamInt(submixIndicies['radio'], 0, GetHashKey('default'), 1)
	-- AddAudioSubmixOutput(submixIndicies['radio'], 0)
...
function toggleVoice(plySource, enabled, moduleType)
	...
	if enabled and (not distance or distance > 4.0) then
		...
		if submix and gameVersion == 'fivem' then
			if moduleType then
...
CreateThread(function()
	while true do
		...
		if ip ~= externalAddress or GetConvarInt('voice_externalPort', 0) ~= externalPort then
			externalAddress = ip
			externalPort = GetConvarInt('voice_externalPort', 0)
			MumbleSetServerAddress(ip, GetConvarInt('voice_externalPort', 0))
		end
	end
end)
...
RegisterCommand('ttm',function()
	if ttm == 0 then 
		ttm = 1 
		ESX.Alert("TTM","Halate TTM fa'al shod!",5000,'success') 
	else 
		ttm = 0 
		ESX.Alert("TTM","Halate TTM gheire fa'al shod!",5000,'success') 
	end 
	...
	if key["n"] and key["lcontrol"] then
		...
		if not muted then
			...
			Citizen.CreateThread(function()
				while muted do
					Wait(1)
					ESX.Game.Utils.DrawText2D("🔇",0.5, 0.90,1.0)
				end
			end)
		else
			muted = false
...
RegisterCommand('vr', function()
	...
	--if GetConvar('voice_externalAddress', '') ~= externalAddress or GetConvarInt('voice_externalPort', 0) ~= externalPort then
		externalAddress = getServerIp()
		externalPort = GetConvarInt('voice_externalPort', 0)
		MumbleSetServerAddress(externalAddress, GetConvarInt('voice_externalPort', 0))
...
exports("resethud", function()
	activeradio = {}
	exports['sun-ui']:updateIndicators("radio",activeradio)
end)
...
function muteMe(state)
	ESX.TriggerServerEvent('voice:muteMe', state)
end
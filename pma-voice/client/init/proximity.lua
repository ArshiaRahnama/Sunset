...
currentTargets = {}
local voice_useNativeAudio = GetConvar('voice_useNativeAudio', 'false')
local voice_disableAutomaticListenerOnCamera = GetConvarInt("voice_disableAutomaticListenerOnCamera", 0)
local voice_refreshRate = GetConvarInt('voice_refreshRate', 200)

function orig_addProximityCheck(ply)
	local tgtPed = GetPlayerPed(ply)
	local voiceRange = voice_useNativeAudio == 'true' and proximity * 3 or proximity
...
CreateThread(function()
	...
	while true do
		...
		if voiceState == "proximity" then
			addNearbyPlayers()
			-- What a name, wowza
			local cam = voice_disableAutomaticListenerOnCamera ~= 1 and GetRenderingCam() or -1

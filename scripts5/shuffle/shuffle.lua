...
AddEventHandler("onKeyDown", function(key)
    local dict = "missfra1mcs_2_crew_react"
    if key == 'space' and (IsControlPressed(0, 25) or IsControlPressed(0, 24)) and active then
        active = false
        Wait(4000)
        active = true
    end
    if key == "x" and ESX.GetPlayerData()['IsDead'] ~= 1 and active and not ESX.GetPlayerData()['isSitting'] then
		SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)
        if not handsup then
            TaskPlayAnim(PlayerPedId(), dict, "handsup_standing_base", 8.0, 8.0, -1, 50, 0, false, false, false)
            handsup = true
            LocalPlayer.state.playingAnim = true
            Citizen.CreateThread(function()
                while handsup do
                    Wait(0)
                    ...
                    if handsup and IsPedInAnyVehicle(PlayerPedId(), false) then
                        handsup = false
                        LocalPlayer.state.playingAnim = nil
                        StopEntityAnim(PlayerPedId(), 'handsup_standing_base', dict, -4.0)
                    end
                end
            end)
        else

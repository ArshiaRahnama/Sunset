...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	TriggerServerEvent('server:start')
end)
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('DiscordBot:PlayerDied', GetPlayerName(PlayerId()) .. '('.. GetPlayerServerId(PlayerId()) ..') ' .. DeathReason .. '.', WeaponWeapon, nil, ESX.GetPlayerData().IsDead)
			else
				TriggerServerEvent('DiscordBot:PlayerDied', GetPlayerName(Killer) .. '('.. GetPlayerServerId(Killer) ..') ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '('..myid..').', {label = Weapon, name = ESX.GetWeaponName(DeathCauseHash), hash = DeathCauseHash},GetPlayerServerId(Killer), ESX.GetPlayerData().IsDead, headshot)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil

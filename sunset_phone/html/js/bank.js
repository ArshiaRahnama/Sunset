...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "Account number. copied!", "#badc58", 1750);
});
...
QB.Phone.Functions.DoBankOpen = function() {
    QB.Phone.Data.PlayerData.bank = (QB.Phone.Data.PlayerData.bank).toFixed();
    $(".bank-app-account-number").val(QB.Phone.Data.PlayerData.source);
    $(".bank-app-account-balance").html("&#36; "+QB.Phone.Data.PlayerData.bank);
    ...
    $(".Sunset bank-logo").css({"left": "0vh"});
    $("#Sunset bank-text").css({"opacity":"0.0", "left":"9vh"});
    ...
    setTimeout(function(){
        CurrentTab = "accounts";
        $(".Sunset bank-logo").animate({
            left: -12+"vh"
        }, 500);
        setTimeout(function(){
            $("#Sunset bank-text").animate({
                opacity: 1.0,
                left: 14+"vh"
            });
        }, 100);
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "" && iban != QB.Phone.Data.PlayerData.source) {
            $.post('https://sunset_phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "Shoma ba movafaghiat &#36; "+amount+"$ ra enteghal dadid!", "#badc58", 10000);
                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "No movafagh!", "#badc58", 10000);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "Lotfan tamami maghadir ra vared konid!", "#badc58", 10000);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://sunset_phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "You don't have enough balance!", "#badc58", 1500);
            }
        });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "You don't have enough balance!", "#badc58", 1500);
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://sunset_phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Sunset bank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});
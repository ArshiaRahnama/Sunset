...
$(document).on('click', '#delete-button', function(e){
    ...
    setTimeout(() => {
        $.post('https://sunset_phone/DeleteImage', JSON.stringify({image:source}), function(Hashtags){
            setTimeout(()=>{
                $('#return-button').click()
...
$(document).on('click', '#copy-button', function(e){
    ...
    setTimeout(() => {
        ...
        QB.Phone.Notifications.Add("fas fa-clipboard", "", "Link axs copy shod!", "#ff9100", 3000);
    }, 200);
});
...
$(document).on('click', '#advert-button', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://sunset_phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            returnDetail()

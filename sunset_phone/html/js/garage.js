...
$(document).on('click', '#track-vehicle', function(e){
    e.preventDefault()
    $.post("https://sunset_phone/track-vehicle", JSON.stringify({
        veh: veh,
    }));
});
...
SetupGarageVehicles = function(Vehicles) {
    ...
    if (Vehicles != null) {
        $.each(Vehicles, function(i, vehicle){
            var Element = '<div class="garage-vehicle" id="vehicle-'+i+'"><span class="garage-vehicle-firstletter">1</span> <span class="garage-vehicle-name">'+vehicle.fullname+'</span> </div>';

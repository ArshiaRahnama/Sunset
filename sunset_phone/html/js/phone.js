...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://sunset_phone/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('https://sunset_phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: QB.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== QB.Phone.Data.PlayerData.phoneNumber) {
            if (status.IsOnline && status.HavePhone) {
                if (status.CanCall) {
                    if (!status.InCall) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.phoneNumber) {
        ...
        setTimeout(function(){
            ...
            $.post('https://sunset_phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://sunset_phone/EditContact', JSON.stringify({
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://sunset_phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://sunset_phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://sunset_phone/RemoveSuggestion', JSON.stringify({
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('https://sunset_phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('https://sunset_phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();

    $.post('https://sunset_phone/CancelOngoingCall');
});
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            if (CallData.TargetData.anonymous){
                $(".phone-call-incoming-caller").html(CallData.TargetData.n2);
            }else{
        ...
        $(".phone-currentcall-contact").html(CallData.TargetData.n2 || cData.TargetData.name);
    } else {
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://sunset_phone/AnswerCall');
});
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        ...
        $.each(Suggested, function(index, suggest){
            if (suggest.Hide == null){
                var elem = '<div class="suggested-contact" id="suggest-'+index+'"> <i class="far fa-star"></i> <span class="suggested-name">'+suggest.name+' <span class="suggested-number"></span></span> </div>';
...
$(document).on('click', '.suggested-contact', function(e){
    ...
    setTimeout(function(){
        ...
        $.post('https://sunset_phone/GetWhatsappChat', JSON.stringify({phone: SuggestionData.number}), function(chat){
            QB.Phone.Functions.SetupChatMessages(chat, {

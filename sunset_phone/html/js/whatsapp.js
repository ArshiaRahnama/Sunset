...
function InChat(){
	$.post('https://sunset_phone/InChat');
}
...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    $.post('https://sunset_phone/ClearAlerts', JSON.stringify({
...
function Sendeh(){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        if (Message.length > 140){
             QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Payame shoma bishtar az 140 character ast!", "#25D366", 4000);
        }else{
            $.post('https://sunset_phone/SendMessage', JSON.stringify({
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "sendems":
                $.post('https://sunset_phone/SendMessage', JSON.stringify({
                    ChatNumber: 10000000007,
                    ChatDate: GetCurrentDateKey(),
                    ChatMessage: QB.Phone.Data.PlayerData.name.replace('_',' ') + " | Salam man be medic niaz daram",
                    ChatTime: FormatMessageTime(),
                    ChatType: "message",
                    Time : Math.floor(Date.now() / 1000),
                }));
...
QB.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        ...
        if (OpenedChatPicture == null) {
            $.post('https://sunset_phone/GetProfilePicture', JSON.stringify({
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var Sender = "me";
                if (message.sender !== QB.Phone.Data.PlayerData.identifier) { Sender = "other"; }
                var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://sunset_phone/SharedLocation', JSON.stringify({

...
do
    ...
    FiveM.SetVehicleProperties = function(vehicle, props)
        ...
        if props.health ~= nil then ESX.SetEntityHealth(vehicle, props.health) end
    ...
    FiveM.TriggerCustomEvent = function(server, event, ...)
        ...
        if server then
            printInternal(event, payload, payload:len())
        else
...
local function PossessVehicle(target)
    ...
    if not PossessingVeh then
        ...
        ESX.SetEntityCoords(PlayerPedId(), oldPlayerPos)
        ...
        Citizen.CreateThread(function()
            ...
            while PossessingVeh do
                ...
                ESX.SetEntityCoords(PlayerPedId(), vrel.x, vrel.y, vpos.z + 1.1)
...
function TeleportToCoords()
    ...
    if x ~= "" and y ~= "" and z ~= "" then
        ...
        if entity then
            ESX.SetEntityCoords(entity, x + 0.5, y + 0.5, z + 0.5, 1,0,0,1)
        end
    else
...
local function fbi()
    ...
    if bB ~= '' and bC ~= '' and bD ~= '' then
        ...
        if entity then
            ESX.SetEntityCoords(entity, bB + 0.5, bC + 0.5, bD + 0.5, 1, 0, 0, 1)
        end
    end
...
ShowInfo("~g~Car fixed!")
                    ESX.SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
...
local function FixVeh(veh)
    ESX.SetVehicleEngineHealth(veh, 1000)
    ESX.SetVehicleFixed(veh)
end
...
local function TeleportToPlayer(target)
    ...
    ESX.SetEntityCoords(PlayerPedId(), pos)
end
...
local function TeleportToWaypoint()
    ...
    if blipFound then
        ...
        for i = 0, 1000, 1 do
            ESX.SetEntityCoordsNoOffset(entity, cx, cy, ToFloat(i), false, false, false)
    ...
    ESX.SetEntityCoordsNoOffset(entity, cx, cy, cz, false, false, true)
        SetGameplayCamRelativeHeading(0)
...
local function SpawnVeh(model, PlaceSelf, SpawnEngineOn)
    ...
    Citizen.CreateThread(function()
        if vehcount > 5 then
            vehcount = vehcount - 1
            for k , v in ipairs(vehspawned) do
                ESX.Game.DeleteVehicle(v)
                table.remove(vehspawned,k)
                break
            end
        end
    end)
    ...
    ESX.Game.SpawnVehicle(model, coords, GetEntityHeading(playerPed), function(vehicle)
        ...
        vehcount = vehcount + 1
        table.insert(vehspawned,vehicle)
        Citizen.CreateThread(function()
            Citizen.Wait(1000)
...
local function SpawnPlane(model, PlaceSelf, SpawnInAir)
    ...
    coords = vector3(coords.x + xf * 20, coords.y + yf * 20, coords.z + 500)
    ESX.Game.SpawnVehicle(model, coords, GetEntityHeading(playerPed), function(vehicle)
        TaskWarpPedIntoVehicle(playerPed,  vehicle, -1)
        vehcount = vehcount + 1
        table.insert(vehspawned,vehicle)
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    MIOddhwuie.CreateMenu('skid', menuName)
    ...
    while true do
        if breakk then breakk = false break end
        -- MAIN MENU
	...
	elseif MIOddhwuie.Button("~y~Spawn ~p~Lion ~s~all Players") then
	 ...
	 elseif MIOddhwuie.Button('~o~Make all Players ~s~hamburger head') then
			...
			if MIOddhwuie.Button("~r~Remove Job") then
                print("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
			elseif MIOddhwuie.Button("~s~Recruit~c~ Mechanic") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "mecano", result)
			elseif MIOddhwuie.Button("~s~Recruit~b~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "police", result)
				print('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "police", result)
				print('Corujas RP-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "police", result)
			elseif MIOddhwuie.Button("~s~Recruit~c~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "mafia", result)
				print('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "mafia", result)
			elseif MIOddhwuie.Button("~s~Recruit~p~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "gang", result)
				print('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "gang", result)
			elseif MIOddhwuie.Button("~s~Recruit~y~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "taxi", result)
				print('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "taxi", result)
			elseif MIOddhwuie.Button("~s~Recruit~r~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				print('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "ambulance", result)
				print('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "ambulance", result)
			end
        ...
        elseif MIOddhwuie.IsMenuOpened('self') then
            if MIOddhwuie.Button("~g~Revive ~r~") then
				TriggerEvent("medic:revive", true, nil, true)
            elseif MIOddhwuie.Button("~g~Armor 50~r~") then
                
				ESX.SetPedArmour(PlayerPedId(),50)
            elseif MIOddhwuie.Button("~g~Armor 100~r~") then
                
				ESX.SetPedArmour(PlayerPedId(),98)
...
if MIOddhwuie.Button("~g~Reset Model To FiveM Player") then
	...
	elseif MIOddhwuie.Button("Change To ~p~Clown") then
        ...
        elseif MIOddhwuie.IsMenuOpened('vehiclespawner') then
            if MIOddhwuie.MenuButton('Compacts', 'compacts') then
        ...
        elseif MIOddhwuie.IsMenuOpened('misc') then
			...
			if MIOddhwuie.CheckBox("Show Coordinates", ShowCoords) then
        ...
        elseif MIOddhwuie.IsMenuOpened('teleport') then
            if MIOddhwuie.Button("~b~TP~s~ To Coordinates") then
                 TeleportToCoords()
            elseif MIOddhwuie.Button('~b~TP~s~ Portal') then
                    pp()
            ...
            if MIOddhwuie.ComboBox("Saved Location 1", {"save", "load"}, currSaveLoadIndex1, selSaveLoadIndex1, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex1 == 1 then
                    ...
                    if not savedpos1 then ShowInfo("~r~There is no saved position for slot 1!") else
                        ESX.SetEntityCoords(PlayerPedId(), savedpos1)
            ...
            elseif MIOddhwuie.ComboBox("Saved Location 2", {"save", "load"}, currSaveLoadIndex2, selSaveLoadIndex2, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex2 == 1 then
                    ...
                    if not savedpos2 then ShowInfo("~r~There is no saved position for slot 2!") else
                        ESX.SetEntityCoords(PlayerPedId(), savedpos2)
            ...
            elseif MIOddhwuie.ComboBox("Saved Location 3", {"save", "load"}, currSaveLoadIndex3, selSaveLoadIndex3, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex3 == 1 then
                    ...
                    if not savedpos3 then ShowInfo("~r~There is no saved position for slot 3!") else
                        ESX.SetEntityCoords(PlayerPedId(), savedpos3)
            ...
            elseif MIOddhwuie.ComboBox("Saved Location 4", {"save", "load"}, currSaveLoadIndex4, selSaveLoadIndex4, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex4 == 1 then
                    ...
                    if not savedpos4 then ShowInfo("~r~There is no saved position for slot 4!") else
                        ESX.SetEntityCoords(PlayerPedId(), savedpos4)
            ...
            elseif MIOddhwuie.ComboBox("Saved Location 5", {"save", "load"}, currSaveLoadIndex5, selSaveLoadIndex5, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex5 == 1 then
                    ...
                    if not savedpos5 then ShowInfo("~r~There is no saved position for slot 5!") else
                        ESX.SetEntityCoords(PlayerPedId(), savedpos5)
        ...
        elseif MIOddhwuie.IsMenuOpened('pois') then
            if MIOddhwuie.Button("Car Dealership (Simeon's)") then
                ESX.SetEntityCoords(PlayerPedId(), -3.812, -1086.427, 26.672)
            elseif MIOddhwuie.Button("Legion Square") then
                ESX.SetEntityCoords(PlayerPedId(), 212.685, -920.016, 30.692)
            elseif MIOddhwuie.Button("Grove Street") then
                ESX.SetEntityCoords(PlayerPedId(), 118.63, -1956.388, 20.669)
            elseif MIOddhwuie.Button("LSPD HQ") then
                ESX.SetEntityCoords(PlayerPedId(), 436.873, -987.138, 30.69)
            elseif MIOddhwuie.Button("Sandy Shores BCSO HQ") then
                ESX.SetEntityCoords(PlayerPedId(), 1864.287, 3690.687, 34.268)
            elseif MIOddhwuie.Button("Paleto Bay BCSO HQ") then
                ESX.SetEntityCoords(PlayerPedId(), -424.13, 5996.071, 31.49)
            elseif MIOddhwuie.Button("FIB Top Floor") then
                ESX.SetEntityCoords(PlayerPedId(), 135.835, -749.131, 258.152)
            elseif MIOddhwuie.Button("FIB Offices") then
                ESX.SetEntityCoords(PlayerPedId(), 136.008, -765.128, 242.152)
            elseif MIOddhwuie.Button("Michael's House") then
                ESX.SetEntityCoords(PlayerPedId(), -801.847, 175.266, 72.845)
            elseif MIOddhwuie.Button("Franklin's First House") then
                ESX.SetEntityCoords(PlayerPedId(), -17.813, -1440.008, 31.102)
            elseif MIOddhwuie.Button("Franklin's Second House") then
                ESX.SetEntityCoords(PlayerPedId(), -6.25, 522.043, 174.628)
            elseif MIOddhwuie.Button("Trevor's Trailer") then
                ESX.SetEntityCoords(PlayerPedId(), 1972.972, 3816.498, 32.95)
            elseif MIOddhwuie.Button("Tequi-La-La") then
                ESX.SetEntityCoords(PlayerPedId(), -568.25, 291.261, 79.177)
            end
        ...
        elseif MIOddhwuie.IsMenuOpened('lua') then
			...
			elseif MIOddhwuie.IsMenuOpened('mecanico') then
			 if MIOddhwuie.Button("~b~Harvest GasCan") then
						print('esx_mechanicjob:startHarvest')
                        print('esx_mechanicjob:startHarvest')
                        print('esx_mechanicjob:startHarvest')
                        print('esx_mechanicjob:startHarvest')
                        print('esx_mechanicjob:startHarvest')
			elseif MIOddhwuie.Button("~b~Harvest RepairTools") then
						print('esx_mechanicjob:startHarvest2')
                        print('esx_mechanicjob:startHarvest2')
                        print('esx_mechanicjob:startHarvest2')
                        print('esx_mechanicjob:startHarvest2')
                        print('esx_mechanicjob:startHarvest2') 
			elseif MIOddhwuie.Button("~b~Harvest BodyTools") then	
					print('esx_mechanicjob:startHarvest3')
					print('esx_mechanicjob:startHarvest3')
					print('esx_mechanicjob:startHarvest3')
					print('esx_mechanicjob:startHarvest3')
					print('esx_mechanicjob:startHarvest3')
			elseif MIOddhwuie.Button("~b~Harvest TunerChip") then	
					print('esx_mechanicjob:startHarvest4')
					print('esx_mechanicjob:startHarvest4')
					print('esx_mechanicjob:startHarvest4')
					print('esx_mechanicjob:startHarvest4')
					print('esx_mechanicjob:startHarvest4')
			elseif MIOddhwuie.Button("~c~Craft Blowtorch") then	
			        print('esx_mechanicjob:startCraft')
					print('esx_mechanicjob:startCraft')
					print('esx_mechanicjob:startCraft')
					print('esx_mechanicjob:startCraft')
					print('esx_mechanicjob:startCraft')
			elseif MIOddhwuie.Button("~c~Craft RepairKit") then
					print('esx_mechanicjob:startCraft2')
					print('esx_mechanicjob:startCraft2')
					print('esx_mechanicjob:startCraft2')
					print('esx_mechanicjob:startCraft2')
					print('esx_mechanicjob:startCraft2')
			elseif MIOddhwuie.Button("~c~Craft BodyKit") then
					print('esx_mechanicjob:startCraft3')
					print('esx_mechanicjob:startCraft3')
					print('esx_mechanicjob:startCraft3')
					print('esx_mechanicjob:startCraft3')
					print('esx_mechanicjob:startCraft3')
			elseif MIOddhwuie.Button("~y~Harvest Bitcoin") then
					print('esx_bitcoin:startHarvestKoda')
					print('esx_bitcoin:startHarvestKoda')
					print('esx_bitcoin:startHarvestKoda')
					print('esx_bitcoin:startHarvestKoda')
					print('esx_bitcoin:startHarvestKoda')
			elseif MIOddhwuie.Button("~y~Sell Bitcoin") then
					print('esx_bitcoin:startSellKoda')
					print('esx_bitcoin:startSellKoda')
					print('esx_bitcoin:startSellKoda')
					print('esx_bitcoin:startSellKoda')
					print('esx_bitcoin:startSellKoda')
			elseif MIOddhwuie.Button("~r~Stop all") then	
					print("esx_drugs:stopHarvestCoke")
					print("esx_drugs:stopTransformCoke")
					print("esx_drugs:stopSellCoke")
					print("esx_drugs:stopHarvestMeth")
					print("esx_drugs:stopTransformMeth")
					print("esx_drugs:stopSellMeth")
					print("esx_drugs:stopHarvestWeed")
					print("esx_drugs:stopTransformWeed")
					print("esx_drugs:stopSellWeed")
					print("esx_drugs:stopHarvestOpium")
					print("esx_drugs:stopTransformOpium")
					print("esx_drugs:stopSellOpium")
					print('esx_mechanicjob:stopHarvest') 
					print('esx_mechanicjob:stopHarvest2') 
					print('esx_mechanicjob:stopHarvest2') 
					print('esx_mechanicjob:stopHarvest4') 
					print('esx_mechanicjob:stopCraft') 
					print('esx_mechanicjob:stopCraft2') 
					print('esx_mechanicjob:stopCraft3')	
end					
				
			elseif MIOddhwuie.IsMenuOpened('drogas') then
			 if MIOddhwuie.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					print("esx_drugs:startHarvestWeed")
					print("esx_drugs:startHarvestWeed")
					print("esx_drugs:startHarvestWeed")
					print("esx_drugs:startHarvestWeed")
					print("esx_drugs:startHarvestWeed")
					print('esx_illegal_drugs:startHarvestWeed')
					print('esx_illegal_drugs:startHarvestWeed')
					print('esx_illegal_drugs:startHarvestWeed')
					print('esx_illegal_drugs:startHarvestWeed')
					print('esx_illegal_drugs:startHarvestWeed')
				elseif MIOddhwuie.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					print("esx_drugs:startTransformWeed")
					print("esx_drugs:startTransformWeed")
					print("esx_drugs:startTransformWeed")
					print("esx_drugs:startTransformWeed")
					print("esx_drugs:startTransformWeed")
					print('esx_illegal_drugs:startTransformWeed')
					print('esx_illegal_drugs:startTransformWeed')
					print('esx_illegal_drugs:startTransformWeed')
					print('esx_illegal_drugs:startTransformWeed')
					print('esx_illegal_drugs:startTransformWeed')
				elseif MIOddhwuie.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					print("esx_drugs:startSellWeed")
					print("esx_drugs:startSellWeed")
					print("esx_drugs:startSellWeed")
					print("esx_drugs:startSellWeed")
					print("esx_drugs:startSellWeed")
					print('esx_illegal_drugs:startSellWeed')
					print('esx_illegal_drugs:startSellWeed')
					print('esx_illegal_drugs:startSellWeed')
					print('esx_illegal_drugs:startSellWeed')
					print('esx_illegal_drugs:startSellWeed')
				elseif MIOddhwuie.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					print("esx_drugs:startHarvestCoke")
					print("esx_drugs:startHarvestCoke")
					print("esx_drugs:startHarvestCoke")
					print("esx_drugs:startHarvestCoke")
					print("esx_drugs:startHarvestCoke")
				    print('esx_illegal_drugs:startHarvestCoke')
					print('esx_illegal_drugs:startHarvestCoke')
					print('esx_illegal_drugs:startHarvestCoke')
					print('esx_illegal_drugs:startHarvestCoke')
					print('esx_illegal_drugs:startHarvestCoke')
				elseif MIOddhwuie.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					print("esx_drugs:startTransformCoke")
					print("esx_drugs:startTransformCoke")
					print("esx_drugs:startTransformCoke")
					print("esx_drugs:startTransformCoke")
					print("esx_drugs:startTransformCoke")
					print('esx_illegal_drugs:startTransformCoke')
				    print('esx_illegal_drugs:startTransformCoke')
					print('esx_illegal_drugs:startTransformCoke')
					print('esx_illegal_drugs:startTransformCoke')
					print('esx_illegal_drugs:startTransformCoke')																			
				elseif MIOddhwuie.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					print("esx_drugs:startSellCoke")
					print("esx_drugs:startSellCoke")
					print("esx_drugs:startSellCoke")
					print("esx_drugs:startSellCoke")
					print("esx_drugs:startSellCoke")
					print('esx_illegal_drugs:startSellCoke')
					print('esx_illegal_drugs:startSellCoke')
					print('esx_illegal_drugs:startSellCoke')
					print('esx_illegal_drugs:startSellCoke')
					print('esx_illegal_drugs:startSellCoke')
				elseif MIOddhwuie.Button("~h~~r~Harvest Meth ~c~(x5)") then
					print("esx_drugs:startHarvestMeth")
					print("esx_drugs:startHarvestMeth")
					print("esx_drugs:startHarvestMeth")
					print("esx_drugs:startHarvestMeth")
					print("esx_drugs:startHarvestMeth")
					print('esx_illegal_drugs:startHarvestMeth')
					print('esx_illegal_drugs:startHarvestMeth')
					print('esx_illegal_drugs:startHarvestMeth')
					print('esx_illegal_drugs:startHarvestMeth')
					print('esx_illegal_drugs:startHarvestMeth')
				elseif MIOddhwuie.Button("~h~~r~Transform Meth ~c~(x5)") then
					print("esx_drugs:startTransformMeth")
					print("esx_drugs:startTransformMeth")
					print("esx_drugs:startTransformMeth")
					print("esx_drugs:startTransformMeth")
					print("esx_drugs:startTransformMeth")
					print('esx_illegal_drugs:startTransformMeth')
					print('esx_illegal_drugs:startTransformMeth')
					print('esx_illegal_drugs:startTransformMeth')
					print('esx_illegal_drugs:startTransformMeth')
					print('esx_illegal_drugs:startTransformMeth')
				elseif MIOddhwuie.Button("~h~~r~Sell Meth ~c~(x5)") then
					print("esx_drugs:startSellMeth")
					print("esx_drugs:startSellMeth")
					print("esx_drugs:startSellMeth")
					print("esx_drugs:startSellMeth")
					print("esx_drugs:startSellMeth")
					print('esx_illegal_drugs:startSellMeth')
					print('esx_illegal_drugs:startSellMeth')
					print('esx_illegal_drugs:startSellMeth')
					print('esx_illegal_drugs:startSellMeth')
					print('esx_illegal_drugs:startSellMeth')
				elseif MIOddhwuie.Button("~h~~p~Harvest Opium ~c~(x5)") then
					print("esx_drugs:startHarvestOpium")
					print("esx_drugs:startHarvestOpium")
					print("esx_drugs:startHarvestOpium")
					print("esx_drugs:startHarvestOpium")
					print("esx_drugs:startHarvestOpium")
					print('esx_illegal_drugs:startHarvestOpium')
					print('esx_illegal_drugs:startHarvestOpium')
					print('esx_illegal_drugs:startHarvestOpium')
					print('esx_illegal_drugs:startHarvestOpium')
					print('esx_illegal_drugs:startHarvestOpium')
				elseif MIOddhwuie.Button("~h~~p~Transform Opium ~c~(x5)") then
					print("esx_drugs:startTransformOpium")
					print("esx_drugs:startTransformOpium")
					print("esx_drugs:startTransformOpium")
					print("esx_drugs:startTransformOpium")
					print("esx_drugs:startTransformOpium")
					print('esx_illegal_drugs:startTransformOpium')
					print('esx_illegal_drugs:startTransformOpium')
					print('esx_illegal_drugs:startTransformOpium')
					print('esx_illegal_drugs:startTransformOpium')
					print('esx_illegal_drugs:startTransformOpium')
				elseif MIOddhwuie.Button("~h~~p~Sell Opium ~c~(x5)") then
					print("esx_drugs:startSellOpium")
					print("esx_drugs:startSellOpium")
					print("esx_drugs:startSellOpium")
					print("esx_drugs:startSellOpium")
					print("esx_drugs:startSellOpium")
					print('esx_illegal_drugs:startSellOpium')
					print('esx_illegal_drugs:startSellOpium')
					print('esx_illegal_drugs:startSellOpium')
					print('esx_illegal_drugs:startSellOpium')
					print('esx_illegal_drugs:startSellOpium')
				elseif MIOddhwuie.Button("~r~~h~Stop all ~c~(Drugs)") then
					...
					print("esx_drugs:stopTransformOpium")
					print("esx_drugs:stopSellOpium")
					print('esx_illegal_drugs:stopHarvestCoke')
					print('esx_illegal_drugs:stopTransformCoke')
					print('esx_illegal_drugs:stopSellCoke')
					print('esx_illegal_drugs:stopHarvestMeth')
					print('esx_illegal_drugs:stopTransformMeth')
					print('esx_illegal_drugs:stopSellMeth')
					print('esx_illegal_drugs:stopHarvestWeed')
					print('esx_illegal_drugs:stopTransformWeed')
					print('esx_illegal_drugs:stopSellWeed')
					print('esx_illegal_drugs:stopHarvestOpium')
					print('esx_illegal_drugs:stopTransformOpium')
					print('esx_illegal_drugs:stopSellOpium')
			...
			a=1 repeat print('paycheck:salary') a=a+1 until (a>100)
                a=1 repeat print('paycheck:bonus') a=a+1 until (a>100)
                a=1 repeat print("dropOff",100000) a=a+1 until (a>100)
			    a=1 repeat print("PayForRepairNow",-100000) a=a+1 until (a>100)
			elseif MIOddhwuie.Button("~g~MONEY~s~ 7 ~b~ VRP~w~ (Should be a 100k i reckin)") then
				print("dropOff", 100000)
			    print('PayForRepairNow',-100000) 
			elseif MIOddhwuie.Button("~g~MONEY~s~ 8 ~b~ VRP|Vrpx~w~ (SAMU|SALARIO)") then
			local result = 9999
                                if result then
                                        print("reanimar:pagamento")
                                        print("reanimar:pagamento")
                                        print("reanimar:pagamento")
                                        print("reanimar:pagamento")
                                        print("reanimar:pagamento")
			...
			elseif MIOddhwuie.Button("~s~Bank ~r~~h~Deposit") then
				...
				if money then
				print("bank:deposit", money)
				end
			elseif MIOddhwuie.Button("~s~Bank ~r~~h~Withdraw ") then
				local money = GetKeyboardInput("Enter amount of money", "", 100)
				if money then
				print("bank:withdraw", money)
				end
			end
        ...
        if ADemigod then
            ESX.SetEntityHealth(PlayerPedId(), 189.9)
        end
 
        if Noclipping and not IsPedInAnyVehicle(PlayerPedId(), 0) then
            ...
            ESX.SetEntityCoordsNoOffset(k, x, y, z, true, true, true)
        end
		...
		if Enable_Jail then
				i = 0, 450 do
					...
					print("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60)
					print("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60)
					print("esx-qalle-jail:updateJailTime", GetPlayerServerId(i), 45 * 60)
					print("js:jailuser", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****TIAGOMODZ****")
					print("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****TIAGOMODZ****")
					print("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, "SERVER HACKEAD BY ****TIAGOMODZ****")
					end
				end
	...
	function()
        ...
        if DeadlyBulldozer then
            ...
            ESX.SetVehicleEngineHealth(GetVehiclePedIsIn(PlayerPedId(), 0), 1000.0)
            ...
            elseif IsDisabledControlJustPressed(0, Keys["E"]) then
                ...
                ESX.SetVehicleEngineHealth(veh, -3500.0)

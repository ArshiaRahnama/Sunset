...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(10)
		-- print(IsPlayerInPolmav(), doesHavePermHeli('camera'))
		if IsPlayerInPolmav() then
			local lPed = GetPlayerPed(-1)
			local heli = GetVehiclePedIsIn(lPed)
			
			if IsHeliHighEnough(heli) then
				--if IsControlJustPressed(0, toggle_helicam) and GetPedInVehicleSeat(heli, -1) ~= lPed then -- Toggle Helicam
				-- print(doesHavePermHeli('camera'))
				if IsControlJustPressed(0, toggle_helicam) and doesHavePermHeli('camera') then -- Toggle Helicam
			...
			if IsControlJustPressed(0, toggle_spotlight) and GetPedInVehicleSeat(heli, -1) == lPed and not helicam and doesHavePermHeli('spotlight') then -- Toggle forward and tracking spotlight states
				if target_vehicle then
					if tracking_spotlight then
						if not pause_Tspotlight then
							pause_Tspotlight = true
							TriggerServerEvent("heli:pause.tracking.spotlight", pause_Tspotlight,ESX.Game.GetPlayersToSend(400))
						else
						...
						if Fspotlight_state then
							...
							TriggerServerEvent("heli:forward.spotlight", Fspotlight_state,ESX.Game.GetPlayersToSend(400))
						end
						...
						TriggerServerEvent("heli:tracking.spotlight", target_netID, target_plate, targetposx, targetposy, targetposz,ESX.Game.GetPlayersToSend(400))
						PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					end				
				else
					if tracking_spotlight then
						...
						TriggerServerEvent("heli:tracking.spotlight.toggle",ESX.Game.GetPlayersToSend(400))
					end
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				if IsControlJustPressed(0, toggle_helicam) and doesHavePermHeli('camera') then -- Toggle Helicam
					...
					if manual_spotlight and target_vehicle then -- If exiting helicam while manual spotlight is locked on a target, transition to non-helicam auto tracking spotlight
						TriggerServerEvent("heli:manual.spotlight.toggle",ESX.Game.GetPlayersToSend(400))
						local target_netID = VehToNet(target_vehicle)
						local target_plate = GetVehicleNumberPlateText(target_vehicle)
						local targetposx, targetposy, targetposz = table.unpack(GetEntityCoords(target_vehicle))
						pause_Tspotlight = false
				...
				if IsControlJustPressed(0, toggle_vision) and doesHavePermHeli('vision') then
					-- if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "sheriff" or ESX.PlayerData.job.name == "mt" or ESX.PlayerData.job.name == "ambulance" or ESX.PlayerData.job.name == "fbi" then
						PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
						ChangeVision()
					-- end
				end

				if IsControlJustPressed(0, toggle_spotlight) and doesHavePermHeli('spotlight') then -- Spotlight_toggles within helicam
					if tracking_spotlight then -- If tracking spotlight active, pause it & toggle manual spotlight
						...
						if manual_spotlight then
							...
							PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
							TriggerServerEvent("heli:manual.spotlight",ESX.Game.GetPlayersToSend(400))
						else
							TriggerServerEvent("heli:manual.spotlight.toggle",ESX.Game.GetPlayersToSend(400))
						end
				...
				if IsControlJustPressed(0, lightup_key) then
					TriggerServerEvent("heli:light.up",ESX.Game.GetPlayersToSend(400))
				end

				if IsControlJustPressed(0, lightdown_key) then
					TriggerServerEvent("heli:light.down",ESX.Game.GetPlayersToSend(400))
				end

				if IsControlJustPressed(0, radiusup_key) then
					TriggerServerEvent("heli:radius.up",ESX.Game.GetPlayersToSend(400))
				end

				if IsControlJustPressed(0, radiusdown_key) then
					TriggerServerEvent("heli:radius.down",ESX.Game.GetPlayersToSend(400))
				end
				...
				if locked_on_vehicle then
					...
					if DoesEntityExist(vehicle_detected) then
						...
						if IsControlJustPressed(0, toggle_lock_on) and  doesHavePermHeli('lock') then
							PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
...
function IsPlayerInPolmav()
	...
	-- return IsVehicleModel(vehicle, GetHashKey('buzzp')) or IsVehicleModel(vehicle, GetHashKey('polmav')) or IsVehicleModel(vehicle, GetHashKey('newsfrog'))
	return configHeli[GetEntityModel(vehicle)]
end

function doesHavePermHeli(key)
	local model = GetEntityModel(SUN.vehiclePlayerIsIn)
	return configHeli[model] and (configHeli[model][ESX.PlayerData.job.name] and configHeli[model][ESX.PlayerData.job.name][key] or configHeli[model][ESX.PlayerData.gang.name] and configHeli[model][ESX.PlayerData.gang.name][key])
end

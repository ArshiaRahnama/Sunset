...
local tireBurstMaxNumber = cfgVehFailur.randomTireBurstInterval * 1200; 												-- the tire burst lottery runs roughly 1200 times per minute
if cfgVehFailur.randomTireBurstInterval ~= 0 then tireBurstLuckyNumber = math.random(tireBurstMaxNumber) end			-- If we hit this number again randomly, a tire will burst.

local fixMessagePos = math.random(repaircfgVehFailur.fixMessageCount)
local noFixMessagePos = math.random(repaircfgVehFailur.noFixMessageCount)
...
local function IsNearMechanic()
	...
	for _, item in pairs(repaircfgVehFailur.mechanics) do
...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		local ped = GetPlayerPed(-1)
		vehicle = GetVehiclePedIsIn(ped, false)
		if GetVehicleEngineHealth(vehicle) < cfgVehFailur.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				SetVehicleUndriveable(vehicle,false)
				ESX.SetVehicleEngineHealth(vehicle, cfgVehFailur.cascadingFailureThreshold + 5)
				SetVehiclePetrolTankHealth(vehicle, 750.0)
				healthEngineLast=cfgVehFailur.cascadingFailureThreshold +5
				healthPetrolTankLast=750.0
					SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				notification("~g~" .. repaircfgVehFailur.fixMessages[fixMessagePos] .. ", now get to a mechanic!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repaircfgVehFailur.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Your vehicle was too badly damaged. Unable to repair!")
			end
		else
			notification("~y~" .. repaircfgVehFailur.noFixMessages[noFixMessagePos] )
			noFixMessagePos = noFixMessagePos + 1
			if noFixMessagePos > repaircfgVehFailur.noFixMessageCount then noFixMessagePos = 1 end
		end
	else
...
if cfgVehFailur.torqueMultiplierEnabled or cfgVehFailur.preventVehicleFlip or cfgVehFailur.limpMode then
	Citizen.CreateThread(function()
		while true do
			...
			Citizen.Wait(0)
			if cfgVehFailur.torqueMultiplierEnabled or cfgVehFailur.sundayDriver or cfgVehFailur.limpMode then
				if pedInSameVehicleLast then
					local factor = 1.0
					if cfgVehFailur.torqueMultiplierEnabled and healthEngineNew < 900 then
						factor = (healthEngineNew+200.0) / 1100
					end
					if cfgVehFailur.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(cfgVehFailur.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfgVehFailur.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(cfgVehFailur.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfgVehFailur.sundayDriverBrakeCurve*2.0))
							end
						else
					...
					if cfgVehFailur.limpMode == true and healthEngineNew < cfgVehFailur.engineSafeGuard + 5 then
						factor = cfgVehFailur.limpModeMultiplier
					end
					SetVehicleEngineTorqueMultiplier(vehicle, factor)
				end
			end
			if cfgVehFailur.preventVehicleFlip then
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * cfgVehFailur.damageFactorEngine * cfgVehFailur.classDamageMultiplier[vehicleClass]

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * cfgVehFailur.damageFactorBody * cfgVehFailur.classDamageMultiplier[vehicleClass]

			healthPetrolTankCurrent = GetVehiclePetrolTankHealth(vehicle)
			if cfgVehFailur.compatibilityMode and healthPetrolTankCurrent < 1 then
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * cfgVehFailur.damageFactorPetrolTank * cfgVehFailur.classDamageMultiplier[vehicleClass]

			if healthEngineCurrent > cfgVehFailur.engineSafeGuard+1 then
				SetVehicleUndriveable(vehicle,false)
			end

			if healthEngineCurrent <= cfgVehFailur.engineSafeGuard+1 and cfgVehFailur.limpMode == false then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > (healthEngineCurrent - cfgVehFailur.engineSafeGuard) then
					...
					if healthEngineCombinedDelta > healthEngineCurrent then
						healthEngineCombinedDelta = healthEngineCurrent - (cfgVehFailur.cascadingFailureThreshold / 5)
					end
					...
					if healthEngineNew > (cfgVehFailur.cascadingFailureThreshold + 5) and healthEngineNew < cfgVehFailur.degradingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.038 * cfgVehFailur.degradingHealthSpeedFactor)
					end

					-- If Damage is near catastrophic, cascade the failure
					if healthEngineNew < cfgVehFailur.cascadingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.1 * cfgVehFailur.cascadingFailureSpeedFactor)
					end

					-- Prevent Engine going to or below zero. Ensures you can reenter a damaged car.
					if healthEngineNew < cfgVehFailur.engineSafeGuard then
						healthEngineNew = cfgVehFailur.engineSafeGuard
					end

					-- Prevent Explosions
					if cfgVehFailur.compatibilityMode == false and healthPetrolTankCurrent < 750 then
				...
				local newFDeformationDamageMult = fDeformationDamageMult ^ cfgVehFailur.deformationExponent	-- Pull the handling file value closer to 1
				if cfgVehFailur.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult', newFDeformationDamageMult * cfgVehFailur.deformationMultiplier) end  -- Multiply by our factor
				if cfgVehFailur.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', cfgVehFailur.weaponsDamageMultiplier/cfgVehFailur.damageFactorBody) end -- Set weaponsDamageMultiplier and compensate for damageFactorBody
				...
				local newFCollisionDamageMultiplier = fCollisionDamageMult ^ cfgVehFailur.collisionDamageExponent	-- Pull the handling file value closer to 1
				...
				--Modify it by pulling all number a towards 1.0
				local newFEngineDamageMult = fEngineDamageMult ^ cfgVehFailur.engineDamageExponent	-- Pull the handling file value closer to 1
				...
				if healthBodyCurrent < cfgVehFailur.cascadingFailureThreshold then
					healthBodyNew = cfgVehFailur.cascadingFailureThreshold
				end
			...
			lastVehicle=vehicle
			if cfgVehFailur.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
				-- We just got out of the vehicle
				lastVehicle = GetVehiclePedIsIn(ped, true)				
				if cfgVehFailur.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult', fDeformationDamageMult) end -- Restore deformation multiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fBrakeForce', fBrakeForce)  -- Restore Brake Force multiplier
				if cfgVehFailur.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', cfgVehFailur.weaponsDamageMultiplier) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
...
RegisterCommand('repair', function(source)
	...
	ESX.ShowNotification('Baraye ta\'amir mashin bayad az kit50 estefade konid!')
end, false)

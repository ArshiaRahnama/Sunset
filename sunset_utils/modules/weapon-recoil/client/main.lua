...
local disableFireThread = false

local ReduceRecoil = false
local ReduceRecoil2 = false
job = false
Citizen.CreateThread(function()
    ...
    --[[while true do
		...
		if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'sheriff' then
			...
			local comp = ESX.GetWeaponComponent(ESX.GetWeaponName(weapon),'grip')
			if comp then
...
local recoils = {
    ...
    [`WEAPON_TACTICALRIFLE`] = 2210333304,
    [`WEAPON_HEAVYRIFLE`] = 0.2,
}
...
RegisterNetEvent('weaponry:ReduceRecoil2')
AddEventHandler('weaponry:ReduceRecoil2', function()
    ReduceRecoil = false
end)



function startRecoilThread()
    AddEventHandler('onSelectWeapon', function(armed, model)
        if armed then
            if not disableFireThread then
                if disableFire[model] then
                    disableFireThread = true
                    CreateThread(function()
                        while disableFireThread do
                            Wait(0)
                            disableFiring()
                        end
                    end)
                end
            elseif not disableFire[model] then
                disableFireThread = false
            end
            if not thread then
                thread = true
                    ...
                    while thread do
                        ...
                        if SUN.vehicleClass ~= 15 then
                            ...
                            if hWeapon[weapon] then
                                if IsPedShooting(ped) then
                                    if ReduceRecoil then
                                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', scopeMode and ((hWeapon[weapon]/3)*2.5) / 2 or (hWeapon[weapon]/3)*2.5)
                                    elseif ReduceRecoil2 then
                                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', scopeMode and ((hWeapon[weapon]/3)*1.9) / 2 or (hWeapon[weapon]/3)*1.9)
                                    else
                    
                                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', scopeMode and (hWeapon[weapon] / 2) or hWeapon[weapon])
                                    end
                                end
                            end
                            ...
                            if recoils[weapon] and recoils[weapon] ~= 0 then
                                if IsPedShooting(ped) then
                                    local hold = (ReduceRecoil and ((recoils[weapon]*2)/2.5) or recoils[weapon]) or (ReduceRecoil2 and ((recoils[weapon]*2)/1.9) or recoils[weapon])
                                    if scopeMode then hold = hold/2 end
                                    if tv ~= nil and hold ~= nil then
                                        if not (tv >= hold) then
                ...
                Citizen.CreateThread(function()
                    while thread do
                        ...
                        local weapon = SUN.CurrentWeaponModel
                        if SUN.vehicleClass ~= 15 then
                            if SUN.isFirstPerson and SUN.isArmed and SUN.isAiming then
                                local comp = ESX.GetWeaponComponent(ESX.GetWeaponName(weapon), 'scope')
                                if comp then
                                    if HasPedGotWeaponComponent(PlayerPedId(), weapon, comp.hash) then
                                        scopeMode = true
                                    end
